...
23、禁止爆仓期间取款，因为射单流程链路长，导致无法立刻执行爆仓，存在窗口客户可以取款。但交易系统里的爆仓应该是一个瞬间动作，不应该是一个持续状态。行情波动快，等待后，可能就不爆仓了，或爆仓金额发生变化。射单场景的爆仓，应该先本地MM，然后再去射单。        
